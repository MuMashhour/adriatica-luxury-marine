...
const CTASection = ({ content }: { content: any }) => (
  <section className="py-32 relative overflow-hidden flex items-center justify-center">
    ...
    <div className="max-w-3xl mx-auto px-6 relative z-10 text-center text-white">
      ...
      >
        {content?.cta_title || 'Ready to list your vessel?'}
      </motion.h2>
      <motion.p
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
      ...
      >
        {content?.cta_text || "Join Egypt's most exclusive maritime network."}
      </motion.p>
      <motion.div
      ...
      >
        <button className="bg-white text-primary px-12 py-5 font-bold text-[11px] tracking-widest uppercase rounded-sm hover:scale-105 transition-transform shadow-xl">
          Get an Appraisal
        </button>
        <button className="border border-white/30 text-white px-12 py-5 font-bold text-[11px] tracking-widest uppercase rounded-sm hover:bg-white/10 transition-all backdrop-blur-sm">
          Inquire Anonymously
        </button>
      </motion.div>
    </div>
